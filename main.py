...
# === BARU: COMMAND !link1 (HANYA SUPER-ADMIN) ===
@bot.command(name="link1")
async def link1(ctx):
    if not has_admin_role(ctx.author):
        await ctx.reply("❌ Hanya **SUPER-ADMIN** yang boleh menggunakan command ini.")
        return

    view = discord.ui.View(timeout=120)

    # Tombol ADD → buka link langsung
    add_btn = discord.ui.Button(
        label="ADD",
        style=discord.ButtonStyle.green,
        url="https://www.roblox.com/id/users/9708533944/profile"
    )

    # Tombol COPY LINK → kirim link via DM atau ephemeral
    class CopyLinkButton(discord.ui.Button):
        def __init__(self):
            super().__init__(label="COPY LINK", style=discord.ButtonStyle.grey)

        async def callback(self, interaction: discord.Interaction):
            link = "https://www.roblox.com/id/users/9708533944/profile"
            try:
                await interaction.user.send(f"📋 Link: `{link}`")
                await interaction.response.send_message("✅ Link dikirim ke DM-mu!", ephemeral=True)
            except discord.Forbidden:
                await interaction.response.send_message(f"📋 Link: `{link}`", ephemeral=True)

    view.add_item(add_btn)
    view.add_item(CopyLinkButton())

    await ctx.send(
        "# LINK INVITE HARI INI YA GUYS\n"
        "Silahkan tekan **Add** untuk membuka link add connection, atau **Copy Link**, untuk mengcopy link (Untuk yang pakai Red Finger)",
        view=view
    )
...
async def on_message(message):
    ...
    if message.content.startswith("!!") and has_admin_role(message.author):
        ...
        # Penting: JANGAN panggil bot.process_commands() di sini — karena sudah ada di bawah
        return
    ...
    # ✅ INI SATU-SATUNYA PERUBAHAN: Aktifkan dukungan command @bot.command()
