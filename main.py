...
VIP_LOUNGE_ROLE_ID = 1449683669471199243  # VIP LOUNGE Role ID
...
def extract_all_user_ids(content):
    ...
    # Ambil semua mention: <@123> atau <@!123>
    mentions = re.findall(r'<@!?(\d+)>', content)
    user_ids.extend([int(uid) for uid in mentions])

    # Jika tidak ada mention, coba cari angka ID (untuk DM/private)
    if not user_ids:
        parts = content.split()
        for part in parts[1:]:  # skip "!!1" atau "!!2"
    ...
    if message.content.startswith("!!") and has_admin_role(message.author):
        try:
            ...
            # !!1 @user1 @user2 ...
                ...
                msg_parts = []
                if success_users:
                    msg_parts.append(f"✅ Berhasil beri `{VIP1_ROLE_NAME}` ke: {', '.join(success_users)}")
                if already_have:
                    msg_parts.append(f"ℹ️ Sudah punya role: {', '.join(already_have)}")
                if not_found:
                    msg_parts.append(f"❌ User tidak ditemukan: {len(not_found)} ID")
            ...
            # !!2 @user1 @user2 ...
                ...
                msg_parts = []
                if success_users:
                    msg_parts.append(f"✅ Berhasil beri `{VIP2_ROLE_NAME}` ke: {', '.join(success_users)}")
                if already_have:
                    msg_parts.append(f"ℹ️ Sudah punya role: {', '.join(already_have)}")
                if not_found:
                    msg_parts.append(f"❌ User tidak ditemukan: {len(not_found)} ID")
            ...
            # !!r ...
                ...
                if arg.lower() == "vip1":
                    ...
                    # Coba ekstrak satu user (mention atau ID)
                    user_id = None
                    ...
                    if roles_removed:
                        ...
                        await message.channel.send(f"ℹ️ {target.mention} tidak punya role VIP 1, VIP 2, atau VIP LOUNGE.")
