...
# === VIP CONFIG ===
VIP1_ROLE_NAME = "VIP 1"
VIP2_ROLE_NAME = "VIP 2"
...
def extract_user_id_from_mention(content):
...
async def on_message(message):
    ...
    # --- HANDLE VIP COMMANDS (HANYA UNTUK SUPER-ADMIN) ---
    if message.content.startswith("!!") and has_admin_role(message.author):
        try:
            ...
            if content.startswith("!!1 "):
                user_id = extract_user_id_from_mention(content)
                if not user_id:
                    await message.channel.send("❌ Format salah. Gunakan: `!!1 @user`")
                    return
                target = guild.get_member(user_id)
                if not target:
                    await message.channel.send("❌ User tidak ditemukan di server ini.")
                    return

                role = discord.utils.get(guild.roles, name=VIP1_ROLE_NAME)
                if not role:
                    await message.channel.send(f"❌ Role `{VIP1_ROLE_NAME}` tidak ditemukan!")
                    return

                if role in target.roles:
                    await message.channel.send(f"ℹ️ {target.mention} sudah punya role `{VIP1_ROLE_NAME}`.")
                else:
                    ...
                    await message.channel.send(f"✅ Berhasil berikan `{VIP1_ROLE_NAME}` ke {target.mention}")

            # !!2 @user
            elif content.startswith("!!2 "):
                user_id = extract_user_id_from_mention(content)
                if not user_id:
                    await message.channel.send("❌ Format salah. Gunakan: `!!2 @user`")
                    return
                target = guild.get_member(user_id)
                if not target:
                    await message.channel.send("❌ User tidak ditemukan di server ini.")
                ...
                role = discord.utils.get(guild.roles, name=VIP2_ROLE_NAME)
                if not role:
                    await message.channel.send(f"❌ Role `{VIP2_ROLE_NAME}` tidak ditemukan!")
                    return

                if role in target.roles:
                    await message.channel.send(f"ℹ️ {target.mention} sudah punya role `{VIP2_ROLE_NAME}`.")
                else:
                    await target.add_roles(role)
                    ...
                    await message.channel.send(f"✅ Berhasil berikan `{VIP2_ROLE_NAME}` ke {target.mention}")
            ...
            # !!r ...
                ...
                if arg.lower() == "vip1":
                    ...
                    user_id = extract_user_id_from_mention(arg)
                    if not user_id:
                        await message.channel.send("❌ Format salah. Gunakan: `!!r @user` atau `!!r vip1` / `!!r vip2`")
                        return
                    target = guild.get_member(user_id)
                    if not target:
                    ...
                    roles_removed = []
                    for role_name in [VIP1_ROLE_NAME, VIP2_ROLE_NAME]:
                    ...
                    if roles_removed:
                        ...
                        await message.channel.send(f"ℹ️ {target.mention} tidak punya role VIP 1 atau VIP 2.")
